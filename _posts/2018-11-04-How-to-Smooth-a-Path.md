...
$\lambda$ determines how sensitive the interpolated point $P$ is to the change in $\angle{ABC}$ and $$\| \overrightarrow{ AB } \|$$. So how does the choice of $\lambda$ affect the outcome? A small $\lambda$ probably won't be very useful as it is going to produce something too similar to the original path, whereas a large $\lambda$ will break the interpolation in a different way by overstating the curvature. Somewhere in between lies the optimal value, which in my case turns out to be 0.25.
<p align="center">
