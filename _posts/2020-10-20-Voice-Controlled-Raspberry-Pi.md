...
Kodi also shines in the input department. On top of the keyboard/mouse, web interface or TV remote option, the ```kodi-send``` command lets the user control the player in command line. For example, if we are interested in replicating pressing the play button, we only need to create the following script ```/usr/local/bin/pictrl```
